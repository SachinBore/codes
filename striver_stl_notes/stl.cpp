// striver stl video lecture notes, from code beyond channel playlist
...
int main() {
	...
	pq.push(-5); // by multiplying element with -1 we are making a min priority queue with max priority queue
	pq.push(-2);
	pq.push(-6);
	...
	// returns iterator pointing to the end() if it is not there
